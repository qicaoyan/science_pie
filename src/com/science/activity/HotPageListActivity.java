...
public class HotPageListActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		GetUrl();
		//ComposeUrl(0);
		strUrl = Url.composeMoreHotListUrl(0, 0, 1, 0);
		initVariable();
		initViews();
	...
	private class MyThread implements Runnable
	{
		...
		public void run() {
			...
			try {
				...
				list=json.getListItems(input);
				Collections.sort(list, hot_item_comparator);
	...
	private class MyGetMoreData implements Runnable
	{
		...
		public void run() {
			...
			try {
				...
				Log.v("HotPageListActivity", strUrl);
	...
	private void refreshPage() {
		new AsyncTask<String, Integer, String>() {
			...
			protected String doInBackground(String... params) {
				...
				strUrl = Url.composeMoreHotListUrl(type_id, plate_id, -1, newest_id);
				URL url;
				try {
					//url = new URL(myApplication.ComposeToken(strUrl+Integer.toString(0)));
					url = new URL(strUrl);
					...
					con.connect();
					InputStream input = con.getInputStream();
					List<Map<String,Object>> new_list;
					new_list = json.getListItems(input);
